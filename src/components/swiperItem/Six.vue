<template>
    <div class="page-box">
        ...
        <div class="content-box" :class="{ 'miniprogramTop': isMiniprogram }">
            ...
            >
                <span>支付金额</span>
                <span class="pay-num">{{ shopReport.buyPayedAmt | formatMoney }}</span>
                <span>元</span>
                <!-- 如果使用了现金券 -->
                <template v-if="shopReport.buyPayedDiscAmt > 0">
                    <span class="use-coupon">（ 使用现金券</span>
                    <span class="coupon-num">{{ shopReport.buyPayedDiscAmt | formatMoney }}</span>
                    <span>元 ）</span>
                </template>
            </div>
            <!-- 累计采购商品 -->
            ...
            >
                <span>支付金额</span>
                <span class="color-orange">{{ shopReport.maxMonthPayedAmt | formatMoney }}</span>
                <span>元</span>
            </div>

            <!-- 固定箭头 -->
            <img
                v-if="swiperItemLoaded_5"
...
export default {
    ...
    watch: {
        swiperIndex() {
            setTimeout(() => {
                this.$emit('swiperItemLoaded','swiperItemLoaded_5')
            },6000)                
        },
    },
    computed: {
